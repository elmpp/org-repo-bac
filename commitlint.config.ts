...
function getProjectsFromMoon(
  ...
  type MoonProjectQueryResponse = {
    projects: { alias: string; type: string }[]
  }

  // const moonProjectQueryResponseJson = execSync('pnpm moon query projects --json', {shell: true as any, encoding: 'utf8'})
  const moonProjectQueryResponseJson = execSync(
    'bun moon query projects --json',
    { shell: true as any, encoding: 'utf8' }
  )
  ...
  return (
    moonProjectQueryResponse!.projects
      .filter((p) =>
        selector({
          name: p.alias,
          projectType: p.type,
...
const projectsFromMoon = getProjectsFromMoon(({ name, projectType }) => {
  return true
})
