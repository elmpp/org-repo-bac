...
export const createAddressPackage = (
  addressIns: Address,
  parseParams: InitialiseOptions['parseParams']
) => {
  function doResolve(options: {
  ...
  function doResolve(options: {
    ...
    const tryResolveFromProjectCwd = (
      ident: string,
      projectCwd: string
    ): AddressPathAbsolute | undefined => {
      try {
    ...
    const tryResolveFromCommonAncestor = (
      ident: string,
      projectCwd: string
    ): AddressPathAbsolute | undefined => {
      try {
        const cliPath = require.resolve('@business-as-code/cli', {
          paths: [projectCwd]
        })
    ...
    resolved ??= tryResolveFromCommonAncestor(
    ...
    if (
      (address as AddressDescriptor<'paramIdentPackage'>).parts.descriptor
        .subpath &&
      includeSubpath
    ) {
    ...
    if (resolved) {
      resolved = addressIns.parsePath(
        _normalizeResolved(resolved!.original)
  ...
  */
  ...
  function resolveRoot(options: {
    ...
    checkGroup(options.address, addressIns)
    const pkgPath = addressIns.parsePackage(
      `${options.address.parts.descriptor.identString}/package.json`
    )
  ...
  function clone(
    address: AddressPackage,
    { params, range }: { params?: URLSearchParams; range?: string }
  ): AddressPackage {
  ...
  function identToDescriptor(options: {
    address: AddressPackageIdent
    range: AddressPackageDescriptor['parts']['descriptor']['range']
  }): AddressPackageDescriptor {
