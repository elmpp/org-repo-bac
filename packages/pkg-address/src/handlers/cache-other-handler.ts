...
export const handler: AddressHandler<'cacheOther'> = {
  ...
  parse({ address, arch, pathType }) {
    ...
    const paramIntersection = arrayIntersection(Array.from(params.keys()), [
      'checksum'
    ])
    if (paramIntersection.unmatched.length) {
      throw new Error(
        `Address: unknown params has been supplied with cache address inside 'cache-other-handler'. Allowed params are 'checksum'. Address: '${address}'`
      )
    }
    const paramsSorted = (() => {
      const params = new URLSearchParams(hashParamString ?? '')
