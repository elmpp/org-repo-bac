...
import {
  fromPortablePath,
  toPortablePath
} from './portable-path-posix-absolute-handler'
  ...
  parse({ address, arch, pathType }) {
    if (arch === 'win32') {
      ...
      if (address.match(WINDOWS_PATH_RELATIVE_REGEX)) {
        const suffix =
          address.endsWith('/*') || address.endsWith('*') ? '/*' : undefined
        const original =
          pathType === 'portable' ? fromPortablePath(address, arch) : address
        const originalNormalized = path.win32.normalize(
          original.replace(/\/\*$/, '').replace(/[\\\/]\*?$/, '')
        )
        const handled = toPortablePath(address, arch)
        const normalized = path.posix.normalize(
          handled.replace(/\/\*$/, '').replace(/[\\\/]\*?$/, '')
        ) as PortablePath
