...
import { fail, ok, Result } from "./__types__";
...
export class XfsCacheManager {
  ...
  async checksumMatch({
    ...
    if (prevChecksum.globalVersion !== nextChecksum.globalVersion) {
      return fail({
        error: new Error(
          `Checksum miss: global keys do not match. This normally happens following a global cache invalidation. Existing: '${prevChecksum.toString()}', expected: '${nextChecksum.toString()}' when validating cache entry '${cacheEntry.content._base}'`
        ),
      });
    }
    if (prevChecksum.key !== nextChecksum.key) {
      return fail({
        error: new Error(
          `Checksum miss: checksum keys do not match. Existing: '${prevChecksum.toString()}', expected: '${nextChecksum.toString()}' when validating cache entry '${cacheEntry.content._base}'`
        ),
      });
    }
    return ok(true)
  ...
  async has({ key, namespace }: { key: string; namespace: string }) {
    ...
    const hasContent = await xfs.existsPromise(cacheEntry.content._base.address)
    const hasMeta = await xfs.existsPromise(cacheEntry.meta.main.address)
    if (Boolean(hasContent) !== Boolean(hasMeta)) {
      throw new Error(`XFSCacheManager corruption: uncoupled state '${JSON.stringify(cacheEntry)}'`)
    }
  ...
  async hasNamespace({ namespace }: { namespace: string }) {
    ...
    return xfs.existsPromise(cacheEntry.content._namespaceBase.address);
  }
  ...
  async set({
    ...
    await xfs.writeFilePromise(cacheEntry.meta.main.address, JSON.stringify(sourceEntry.meta), 'utf-8')
  ...
  async getCacheEntry({
    key: unsanitisedKey,
    namespace: unsanitisedNamespace,
  }: {
    key: string;
    namespace: string;
  }): Promise<CacheEntry> {
