...
export function assertIsResult<R, E extends { error: Error }>(
  res: any
): res is Result<R, E> {
...
export type ResultPromiseAugment<
  Res extends Promise<Result<any, any>>,
  Add extends Record<string, unknown>
> = Promise<ResultAugment<UnwrapPromise<Res>, Add>>
export type ResultAugment<
  Res extends Result<any, any>,
  Add extends Record<string, unknown>
> = Res extends Result<infer XOk, infer XFail>
  ? Res extends { success: true }
    ? Ok<XOk & Add>
    : Fail<XFail>
  : never
...
export type InferOkResult<R extends Result<unknown, any>> = Extract<
  R,
  { success: true }
>
