...
export function objectMapAndFilter<Obj extends Record<string, any>, Key extends keyof Obj>(
  obj: Obj,
...
>(obj: Obj, iteratee: (value: Obj[Key], key: Key, idx: number) => Out): {[K in keyof Obj]: Out}
...
>(obj: Obj, iteratee: (value: Obj[Key], key: Key, idx: number) => any | Symbol): Obj
export function objectMapAndFilter<
  Obj extends Record<string, any>,
  Key extends keyof Obj,
  Out
...
/** when the iteratee returns same type as its input (with no skip), the overall returnType can be the same as Obj */
export async function objectMapAndFilterPromise<Obj extends Record<string, any>, Key extends keyof Obj>(
  obj: Obj,
...
>(obj: Obj, iteratee: (value: Obj[Key], key: Key, idx: number) => Promise<Out>): Promise<{[K in keyof Obj]: Out}>
/** when the iteratee returns a type that includes a skip, the overall returnType can be partial (cannot do 'Obj[key]: never' though it appears) */
export async function objectMapAndFilterPromise<
  Obj extends Record<string, any>,
  Key extends keyof Obj
...
>(
  obj: Obj,
  iteratee: any,
): Promise<{[K in keyof Obj]: any | never}> {
  let key: Key // this is how to avoid string inference of keys yay!
  let returnObj = {} as {
    [K in keyof Obj]: Out | never
  }
