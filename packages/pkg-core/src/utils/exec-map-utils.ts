 ...
 */
export const doMapExec = async (options: {
...
): Promise<Result<PmapElementResult<Awaited<ReturnType<typeof doExec>>, Project>[], {error: BacError}>> => {
  return new Promise((resolve) => {
    // if (typeof mapper !== 'function') {
    //   throw new TypeError('Mapper function is required')
    // }

    const { projects, command, execOptions, mapOptions: { concurrency = Infinity, mode = 'allStopOnError' } = {} } = options
    ...
    const result: PmapElementResult<Awaited<ReturnType<typeof doExec>>, Project>[] = []
    // const errors = []
    ...
    const next = () => {
      ...
      resolvingCount++
        ; (async () => {
          if (finishedValue) {
            result[index] = doMapExec.stop(finishedValue.value, nextItem as any, finishedValue.reason) // pad out the response array with our symbol after explicit stop/error
          } else {
...
doMapExec.stop = <Element>(
  ...
  key: Element,
  reason?: string
): PmapElementResultSkipped<Element> => ({ result: 'skipped', key, value, reason: reason ?? 'Process was skipped' })
