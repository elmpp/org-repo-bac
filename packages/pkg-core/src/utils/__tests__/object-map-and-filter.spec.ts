...
describe('objectMapAndFilter', () => {
  describe('standard Record inputs', () => {
    it('maintains types on standard objects when returning same type', async () => {
      const resourceState = objectMapAndFilter(
        mockNormalStateObject,
        (val, key, idx) => {
      ...
      expectTypeOf(
        resourceState.workrootPackageManager.workrootPackageManagerYarn2Pnp
          .enableColors
    ...
    it('maintains types on standard objects when returning different types', async () => {
      ...
      expectTypeOf<typeof resourceState>().not.toEqualTypeOf<
        typeof mockNormalStateObject
      >() // shows that it picks up iteratee return when mapping standard states
    })
    it('maintains types on standard objects when returning different types including functions', async () => {
      const noop = (anArg: string) => {
        return true as any
      }
      const resourceState = objectMapAndFilter(
        mockNormalStateObject,
      ...
      expectTypeOf<ValueOf<typeof resourceState>>().toEqualTypeOf(noop) // resourceState picks up the returnType of the iteratee
    ...
    it('maintains types on standard objects when skipping', async () => {
      ...
      expectTypeOf<typeof resourceState>().toEqualTypeOf<
        typeof mockNormalStateObject
      >() // atm, we don't apply Partial<>
    })
  })
})
...
describe('objectMapAndFilterPromise', () => {
  describe('standard Record inputs', () => {
    it('maintains types on standard objects when returning same type', async () => {
      const resourceState = await objectMapAndFilterPromise(
        mockNormalStateObject,
        async (val, key, idx) => {
          expectTypeOf(key).toEqualTypeOf<
            | 'workrootMeta'
