...
describe("validation utils", () => {
  it("providerOptionsSchemaBuilder", () => {
    const optionsSchema = providerOptionsSchemaBuilder(
      "configureWorkspace",
    );
  ...
  it("returnOptionsArraySchemaBuilder", () => {
    const optionsSchema = providerReturnOptionsSchemaBuilderArray(
      "configureWorkspace",
    );

    type SchemaType = z.infer<typeof optionsSchema>

    expectTypeOf<SchemaType>().toMatchTypeOf<Array<{
      provider: 'git',
      options: {
        address: string,
        // success: boolean,
  ...
  it("returnOptionsSingularSchemaBuilder", () => {
    const optionsSchema = providerReturnOptionsSchemaBuilderSingular(
      "configureWorkspace",
    );
