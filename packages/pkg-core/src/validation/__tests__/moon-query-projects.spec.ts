      ...
      it('validates a single-project moon projectGraph.json file', async () => {
        const projectGraphJson = await xfs.readJsonPromise(
          addr.pathUtils.resolve(
            ...
            addr.parsePath(
              './etc/query/projects/v1.5.1/language=typescript.json'
            )
          ).address
        )
      ...
      it('validates a full moon projectGraph.json file', async () => {
        const projectGraphJson = await xfs.readJsonPromise(
          addr.pathUtils.resolve(
            addr.parsePath(__dirname),
            addr.parsePath(
              './etc/query/projects/v1.5.1/language=typescript-full.json'
            )
          ).address
        )
        // console.log(`projectGraphJson :>> `, projectGraphJson)
      ...
      it('validates a single-project moon projectGraph.json file', async () => {
        const projectGraphJson = await xfs.readJsonPromise(
          addr.pathUtils.resolve(
            addr.parsePath(__dirname),
            addr.parsePath(
              './etc/query/projects/v1.6.0/language=typescript.json'
            )
          ).address
        )
        // console.log(`projectGraphJson :>> `, projectGraphJson)
      ...
      it('validates a full moon projectGraph.json file', async () => {
        const projectGraphJson = await xfs.readJsonPromise(
          addr.pathUtils.resolve(
            addr.parsePath(__dirname),
            addr.parsePath(
              './etc/query/projects/v1.6.0/language=typescript-full.json'
            )
          ).address
        )
        // console.log(`projectGraphJson :>> `, projectGraphJson)
      ...
      it('validates a single-project moon projectGraph.json file', async () => {
        const projectGraphJson = await xfs.readJsonPromise(
          addr.pathUtils.resolve(
            addr.parsePath(__dirname),
            addr.parsePath(
              './etc/query/projects/v1.8.3/language=typescript.json'
            )
          ).address
        )
        // console.log(`projectGraphJson :>> `, projectGraphJson)
      ...
      it('validates a full moon projectGraph.json file', async () => {
        const projectGraphJson = await xfs.readJsonPromise(
          addr.pathUtils.resolve(
            addr.parsePath(__dirname),
            addr.parsePath(
              './etc/query/projects/v1.8.3/language=typescript-full.json'
            )
          ).address
        )
        // console.log(`projectGraphJson :>> `, projectGraphJson)
      ...
      it('validates a single-project moon projectGraph.json file', async () => {
        const projectGraphJson = await xfs.readJsonPromise(
          addr.pathUtils.resolve(
            addr.parsePath(__dirname),
            addr.parsePath(
              './etc/query/projects/v1.9.3/language=typescript.json'
            )
          ).address
        )
        // console.log(`projectGraphJson :>> `, projectGraphJson)
      ...
      it('validates a full moon projectGraph.json file', async () => {
        const projectGraphJson = await xfs.readJsonPromise(
          addr.pathUtils.resolve(
            addr.parsePath(__dirname),
            addr.parsePath(
              './etc/query/projects/v1.9.3/language=typescript-full.json'
            )
          ).address
        )
        // console.log(`projectGraphJson :>> `, projectGraphJson)
      ...
      it('validates a single-project moon projectGraph.json file', async () => {
        const projectGraphJson = await xfs.readJsonPromise(
          addr.pathUtils.resolve(
            addr.parsePath(__dirname),
            addr.parsePath(
              './etc/query/projects/v1.10.1/language=typescript.json'
            )
          ).address
        )
        expect(moonQueryProjects.parse(projectGraphJson)).toBeTruthy()
      ...
      it('validates a full moon projectGraph.json file', async () => {
        const projectGraphJson = await xfs.readJsonPromise(
          addr.pathUtils.resolve(
            addr.parsePath(__dirname),
            addr.parsePath(
              './etc/query/projects/v1.10.1/language=typescript-full.json'
            )
          ).address
        )
        expect(moonQueryProjects.parse(projectGraphJson)).toBeTruthy()
    ...
    it('validates a single-project moon projectGraph.json file', async () => {
      const projectGraphJson = await xfs.readJsonPromise(
        addr.pathUtils.resolve(
          addr.parsePath(__dirname),
          addr.parsePath(
            './etc/query/projects/v1.18.3/language=typescript.json'
          )
        ).address
      )
      expect(moonQueryProjects.parse(projectGraphJson)).toBeTruthy()
    ...
    it('validates a full moon projectGraph.json file', async () => {
      const projectGraphJson = await xfs.readJsonPromise(
        addr.pathUtils.resolve(
          addr.parsePath(__dirname),
          addr.parsePath(
            './etc/query/projects/v1.18.3/language=typescript-full.json'
          )
        ).address
      )
      expect(moonQueryProjects.parse(projectGraphJson)).toBeTruthy()
