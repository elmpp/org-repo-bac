...
const READONLY_IMPLEMENTATIONS = [
  ...READONLY_SYNC_IMPLEMENTATIONS,
  ...READONLY_ASYNC_IMPLEMENTATIONS
]
...
export class ReadOnlyJailFS extends ProxiedFS<PortablePath, PortablePath> {
  ...
  constructor(
    target: PortablePath,
    { baseFs = new NodeFS() }: JailFSOptions = {}
  ) {
    super(ppath)
          ...
          return () =>
            (...args: unknown[]) => {
              if (READONLY_IMPLEMENTATIONS.includes(prop as any)) {
                throw new Error(
                  `You have used a non-permitted FS api method. Only the following may be used: '${READONLY_IMPLEMENTATIONS.join(', ')}'`
                )
              }
  ...
  override getRealPath() {
    return this.pathUtils.resolve(
      ...
      this.pathUtils.relative(PortablePath.root, this.target)
    )
  }
