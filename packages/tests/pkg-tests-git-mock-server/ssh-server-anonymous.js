...
new Promise((resolve, reject) => {
  if (
    ...
    throw new Error(
      `ssh-server-anonymous is not intended for private or password access!!. '${process.env.GIT_SSH_MOCK_SERVER_PUBKEY}', '${process.env.GIT_SSH_MOCK_SERVER_PASSWORD}'`
    )
  }
  ...
  try {
    ...
    try {
      if (!fs.existsSync(path.join(config.root, 'id_rsa'))) {
        console.warn(
          `REGENERATING KEYPAIR TO '${JSON.stringify(keyPaths)}. SHOULD ONLY HAPPEN FIRST EVER RUN! ssh-server-anonymous'`
        )
  ...
  new ssh2.Server(
    ...
    parseInt(
      process.env.GIT_SSH_ANONYMOUS_MOCK_SERVER_PORT ||
