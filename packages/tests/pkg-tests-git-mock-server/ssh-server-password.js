...
new Promise((resolve, reject) => {
  ...
  if ((process.env?.GIT_SSH_MOCK_SERVER_PUBKEY ?? 'false').toUpperCase() === 'true' || (process.env?.GIT_SSH_MOCK_SERVER_PASSWORD ?? 'false').toUpperCase() === 'true') {
    throw new Error(`ssh-server-password is not intended for private or password access!!. '${process.env.GIT_SSH_MOCK_SERVER_PUBKEY}', '${process.env.GIT_SSH_MOCK_SERVER_PASSWORD}'`)
  }
