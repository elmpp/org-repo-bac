...
new Promise((resolve, reject) => {
  try {
    ...
    try {
      if (!fs.existsSync(path.join(config.root, 'id_rsa'))) {
        console.warn(
          `REGENERATING KEYPAIR TO '${JSON.stringify(keyPaths)}. SHOULD ONLY HAPPEN FIRST EVER RUN!'. ssh-server`
        )
        ...
        let proc = spawnSync(
          ...
          {
            // FIXES https://tinyurl.com/24lqexhx
        ...
        console.log(
          `fs.existsSync(path.join(__dirname, 'id_rsa')) :>> `,
          fs.existsSync(path.join(config.root, 'id_rsa'))
        )
      }
  ...
  if (process.argv[2] === 'exportKeys') {
    console.error(
      `Not supported. Do 'ssh-add ${config.root}/id_rsa' / 'ssh-add -d ${config.root}/id_rsa'`
    )
    process.exit(1)
  ...
  new ssh2.Server(
    {
      ...
      // },
      debug: console.info
      // debug: (meh) => process.stdout.write(meh.toString() + "\n"),
    },
    function (client) {
      ...
      client
        .on('authentication', function (ctx) {
          ...
          if (ctx.method === 'publickey') {
            console.log(
              `attempting publickey login :>> '${process.env.GIT_SSH_MOCK_SERVER_PUBKEY}' :: '${ctx.key.algo}' :: '${pubKey.type}' :: '${ctx.key?.data?.toString()}' :: '${pubKey.getPublicSSH().toString()}'`
            )
          }
          if (ctx.method === 'password') {
            console.log(
              `attempting password login :>> '${Buffer.from(ctx.password || '').toString()}' :: '${PASSWORD_BUFFER}' :: '${process.env.GIT_SSH_MOCK_SERVER_PASSWORD}'`
            )
          }
        ...
        .on('ready', function () {
          ...
          client.on('session', function (accept, reject) {
            ...
            session.once('exec', function (accept, reject, info) {
              ...
              console.log(
                `command: ${info.command}, fixtureName: ${fixtureName}, gitdir: ${gitdir}, config.root: ${config.root}, process.cwd(): ${process.cwd()}, f: ${f}`
              )
              let fulldir
              ...
              try {
                ...
                console.log(fulldir + ' does not exist.', err)
                return reject()
              }
    ...
    parseInt(
      process.env.GIT_SSH_PUBKEY_MOCK_SERVER_PORT ||
        constants.GIT_SSH_PUBKEY_MOCK_SERVER_PORT,
      10
    ),
    '127.0.0.1',
    function () {
      console.log(
        'Listening on port ' + process.env.GIT_SSH_PUBKEY_MOCK_SERVER_PORT ||
          constants.GIT_SSH_PUBKEY_MOCK_SERVER_PORT
      )
    }
  )
