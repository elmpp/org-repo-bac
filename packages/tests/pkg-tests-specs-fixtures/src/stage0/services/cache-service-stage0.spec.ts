...
describe('cache-service', () => {
  ...
  it('fetches and stores ok', async () => {
    ...
    await persistentTestEnv.test({}, async (testContext) => {
      ...
      expect(entry.contentPath.original).toEqual(
        expect.stringMatching(
          `/${constants.RC_CONTENT_FOLDER}/portablePathPosixAbsolute/`
        )
      )
      expect(entry.metaPath.original).toEqual(
        expect.stringMatching(
          new RegExp(`/${constants.RC_META_FOLDER}\/.*.json$`)
        )
      )
      expect(entry.metaPathRelative.original).toEqual(
        expect.stringMatching(new RegExp(`[0-9]{4}.json$`))
      )
