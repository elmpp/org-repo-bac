...
import { XfsCacheManager } from '@business-as-code/core/src/cache/xfs-cache-manager'
...
describe('xfs-cache-manager', () => {
  it('sets up ok', async () => {
    const persistentTestEnv = await createPersistentTestEnv({
      testName: 'xfs-cache-manager:sets up ok'
    })
  ...
  it('saves to meta', async () => {
    const persistentTestEnv = await createPersistentTestEnv({
      testName: 'xfs-cache-manager:saves to meta'
    })
    await persistentTestEnv.test({}, async (testContext) => {
      const options = {
        metaBaseAddress: addr.pathUtils.join(
      ...
      expect(
        expectFs.existsSync(
          'meta/a-namespace-that-is-sanitised/a-key-that-is-sanitised/main.json'
        )
      ).toBeTruthy()
      expect(
        expectFs.readJson(
  ...
  it('throws when a key/namespace are not sanitised', async () => {
    const persistentTestEnv = await createPersistentTestEnv({
      testName:
        'xfs-cache-manager:throws when a key/namespace are not sanitised'
    })
    await persistentTestEnv.test({}, async (testContext) => {
      const options = {
        metaBaseAddress: addr.pathUtils.join(
  ...
  it('CacheEntry.content is optional without constructor contentBaseAddress', async () => {
    const persistentTestEnv = await createPersistentTestEnv({
      testName:
        'xfs-cache-manager:CacheEntry.content is optional without constructor contentBaseAddress'
    })
    await persistentTestEnv.test({}, async (testContext) => {
      const options = {
        metaBaseAddress: addr.pathUtils.join(
  ...
  it('CacheEntry.content is NonNullable with constructor contentBaseAddress', async () => {
    const persistentTestEnv = await createPersistentTestEnv({
      testName:
        'xfs-cache-manager:CacheEntry.content is NonNullable with constructor contentBaseAddress'
    })
    await persistentTestEnv.test({}, async (testContext) => {
      const options = {
        metaBaseAddress: addr.pathUtils.join(
  ...
  it('can be set up without content', async () => {
    const persistentTestEnv = await createPersistentTestEnv({
      testName: 'xfs-cache-manager:can be set up without content'
    })
    await persistentTestEnv.test({}, async (testContext) => {
      const options = {
        metaBaseAddress: addr.pathUtils.join(
