  ...
  describe('errors', () => {
    it('nonexistent command is handled and added to stderr', async () => {
      const persistentTestEnv = await createPersistentTestEnv({
        testName: `initialise workspace: nonexistent command is handled and added to stderr`
      })
    ...
    it('incorrect command flags', async () => {
      const persistentTestEnv = await createPersistentTestEnv({
        testName: `initialise workspace: incorrect command flags`
      })
      await persistentTestEnv.test({}, async (testContext) => {
    ...
    it('incorrect command args', async () => {
      const persistentTestEnv = await createPersistentTestEnv({
        testName: `initialise workspace: incorrect command args`
      })
      await persistentTestEnv.test({}, async (testContext) => {
        const res = await testContext.command(
          [
    ...
    it('--workspacePath is required', async () => {
      const persistentTestEnv = await createPersistentTestEnv({
        testName: `initialise workspace: --workspacePath is required`
      })
      await persistentTestEnv.test({}, async (testContext) => {
        const res = await testContext.command(
