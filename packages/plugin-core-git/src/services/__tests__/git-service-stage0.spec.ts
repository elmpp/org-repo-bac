...
import { Result, ServiceMap, UnwrapPromise, constants, expectIsOk } from "@business-as-code/core";
import { Filename, xfs } from "@business-as-code/fslib";
import { TestContext, createPersistentTestEnv } from "@business-as-code/tests-core";
    ...
    it("clone ssh pubkey", async () => {
      ...
      await persistentTestEnv.test({}, async (testContext) => {
        ...
        const res = await service.clone(
          ...
          {sshStrictHostCheckingDisable: false, sshPrivateKeyPath: testContext.testEnvVars.sshPrivateKeyPath.original}
        );
        await assertCommon({testContext, res, service})
      });
    });
  ...
  describe("ls-remote", () => {
    it("remote list", async () => {
      ...
      await persistentTestEnv.test({}, async (testContext) => {
        ...
        expectOut.lineContainsString({match: '\trefs/heads/main', occurrences: 1})
